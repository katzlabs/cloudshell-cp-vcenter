...
class LinkedCloneArtifactHandler(object):
    ...
    def save(self, save_action, cancellation_context):
        ...
        if self.vcenter_data_model.saved_sandbox_storage:
            data_holder.template_resource_model.vm_storage = (
                self.vcenter_data_model.saved_sandbox_storage
            )

        with self._manage_power_during_save(save_action):
            self.logger.info(
                "[{0}] Save sandbox - Creating Source VM".format(thread_id)
            )

            self.logger.info(
                "[{0}] Copying existing sandbox app to create saved app source: \nOriginal app: {1}".format(
        ...
        self.logger.info(
            "[{1}] Successfully cloned an app from sandbox to saved sandbox - saved sandbox id: {0}".format(
                saved_sandbox_id, thread_id
            )
        )
        self.logger.info(
            "[{2}] Saved Sandbox App will clone from VM: {0}\n{1}".format(
                result.vmUuid, result.vmName, thread_id
            )
        )
        ...
        vcenter_vm_path = self._get_saved_app_result_vcenter_vm_path(
            data_holder, result
        )

        saved_entity_attributes = [
    ...
    def _get_saved_app_result_vcenter_vm_path(self, data_holder, result):
        ...
        vm_location_without_datacenter = "/".join(
            vm_location_with_datacenter.split("/")[1:]
        )
    ...
    def delete(self, delete_saved_app_actions, cancellation_context, pool):
        ...
        self.logger.info(
            "Saved sandbox path/s: {0}".format(", ".join(saved_sandbox_paths))
        )
    ...
    def _get_rid_of_vm_if_found(self, params):
        ...
        self.logger.info(
            "Checking if need to dispose of artifact: {0}".format(artifact.artifactRef)
        )
        vm = self.pv_service.get_vm_by_uuid(self.si, artifact.artifactRef)
        if vm:
            self.logger.info(
                "Will dispose {0}, it is a VM".format(artifact.artifactRef)
            )
            ...
            return
        self.logger.info(
            "{0} was not a vm or vm not found".format(artifact.artifactRef)
        )

    def _get_delete_tasks(self, delete_saved_app_actions):
        return [
            DeleteAppTask(action.actionParams.artifacts, action)
    ...
    def destroy(self, save_action):
        ...
        try:
            self.folder_manager.delete_folder_with_vm_power_off(
                self.si, self.logger, saved_sandbox_path
            )
        except:
            self.logger.info(
                "Rollback for save_action {0} failed".format(save_action.actionId)
            )

        self.logger.info(
            "Rollback for save_action {0} successful".format(save_action.actionId)
        )

    def _disconnect_all_quali_created_networks(self, result):
        thread_id = threading.current_thread().ident
        self.logger.info(
            "{0} clearing networks configured by cloudshell on saved sandbox source app {1}".format(
                thread_id, result.vmName
            )
        )
        ...
        self.logger.info(
            "{0} Holding network is {1}".format(thread_id, network_full_name)
        )
        ...
        self.pg_configurer.disconnect_all_networks_if_created_by_quali(
            vm, default_network, self.vcenter_data_model.reserved_networks, self.logger
        )

    def _get_source_vm(self, save_action):
        vm_uuid = save_action.actionParams.sourceVmUuid
        could_not_save_artifact_message = (
            "Could not find VM with uuid {0}. \nCould not save artifact".format(vm_uuid)
        )
        try:
    ...
    def _get_saved_sandbox_path(self, save_action):
        data_holder = self._prepare_vm_data_holder(save_action, self.vcenter_data_model)
        saved_sandbox_id = save_action.actionParams.savedSandboxId
        return self._get_saved_sandbox_id_full_path(
            data_holder.template_resource_model.vm_location, saved_sandbox_id
        )
    ...
    def _update_cloned_vm_target_location(self, data_holder, saved_sandbox_id):
        data_holder.template_resource_model.vm_location = (
            self._vcenter_sandbox_folder_path(saved_sandbox_id, data_holder)
        )

    def _prepare_cloned_vm_vcenter_folder_structure(
        self, data_holder, saved_sandbox_id
    ):
    ...
    def _prepare_vm_data_holder(self, save_action, vcenter_data_model):
        ...
        deploy_from_vm_model.vm_location = (
            self.vcenter_data_model.vm_location or deploy_from_vm_model.vm_location
        )
    ...
    def _generate_cloned_vm_name(self, save_action):
        source_vm = self.pv_service.get_vm_by_uuid(
            self.si, save_action.actionParams.sourceVmUuid
        )
        if not source_vm:
    ...
    def _get_or_create_saved_sandbox_folder(self, saved_sandbox_id, data_holder):
        saved_apps_folder_path = "/".join(
            [data_holder.template_resource_model.vm_location, SAVED_SANDBOXES]
        )
        self.folder_manager.get_or_create_vcenter_folder(
            self.si, self.logger, saved_apps_folder_path, saved_sandbox_id
        )

    def _vcenter_sandbox_folder_path(self, saved_sandbox_id, data_holder):
        vm_location = "/".join(
            data_holder.template_resource_model.vm_location.split("/")[1:]
        )
    ...
    def _get_or_create_saved_apps_folder_in_vcenter(self, data_holder):
        ...
        return self.folder_manager.get_or_create_vcenter_folder(
            self.si, self.logger, root_path, SAVED_SANDBOXES
        )
    ...
    def _manage_power_during_save(self, save_action):
        ...
        power_off_during_clone = self._should_vm_be_powered_off_during_clone(
            save_action
        )
    ...
    def _power_off_vm(self, vm, cancellation_context=None):
        if vm.summary.runtime.powerState != "poweredOff":
            ...
            self.task_waiter.wait_for_task(
                task, self.logger, "Power Off", cancellation_context
            )

    def _delete_vm(self, vm, cancellation_context=None):
        task = vm.Destroy_Task()
        self.task_waiter.wait_for_task(
            task, self.logger, "Delete VM", cancellation_context
        )
    ...
    def _should_vm_be_powered_off_during_clone(self, save_action):
        ...
        behavior_during_save = (
            save_attributes.get("Behavior during save")
            or self.vcenter_data_model.behavior_during_save
        )
