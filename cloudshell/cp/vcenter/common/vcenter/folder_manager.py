...
class FolderManager(object):
    ...
    def delete_folder_with_vm_power_off(self, si, logger, folder_full_path):
        ...
        if not folder:
            logger.info(
                "Could not find {0}, maybe it was already removed?".format(
                    folder_full_path
                )
            )
        ...
        logger.info(
            "Remove result for {0} and all child folders and vms\n{1}".format(
                folder_full_path, result
            )
        )
    ...
    def delete_folder(self, folder, logger):
        ...
        try:
            self.task_waiter.wait_for_task(
                task=task, logger=logger, action_name="Destroy Folder"
            )
            result = SUCCESS
    ...
    def get_or_create_vcenter_folder(self, si, logger, path, folder_name):
        ...
        if not vcenter_folder:
            with self.locks[vcenter_folder_path]:
                ...
                if not vcenter_folder:
                    logger.info(
                        "{0} was not found under {1}, creating...".format(
                            folder_name, path
                        )
                    )
